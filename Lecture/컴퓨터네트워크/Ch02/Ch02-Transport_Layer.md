...
- 코너의 두 끝점을 정의하는 네개의 튜플
- 로컬 IP 주소, 로컬 포트, 외부 IP 주소, 외부 포트
- 모든 TCP연결을 고유하게 힉별
- 각 끝점(IP 주소 및 포트)를 식별하는 두 값을 종종 "소켓"이라고 부른다.

### Buffer Sizes and Limitations
- IP 데이터그램의 최대 크기는 65,535바이트이다.
    - 16 bit 페이로드 필드 길이 때문이다.
- 많은 네트워크에는 HW에 의해 지시될 수 있는 MTU[^MTU]가 있다.
- 경로에서 가장 작은 MTU를 "path MTU"라고 한다.
- 데이터그램 크기가 링크 MTU를 초과할 때 조각화가 수행된다.
- TCP에서 MSS(Max Segment Size)
    - 피어가 세그먼트당 전송할 수 있는 최대 TCP 데이터양(16비트 MSS가 SYN에 있음)
    - MSS는 종종 MTU에서 IP 및 TCP헤더를 뺀 값으로 설정된다.
    - IPv4 또는 IPv6을 사용하는 이더넷의 경우 1460B 또는 1440B

#### TCP output
- 소켓 버퍼에 공간이 부족할 경우 프로세스가 중지(sleep)됨(최종 바이트 작성 후 반환)
![그림6](./그림6.png)

#### UDP output
- 소켓 버퍼가 실제로 존재하지 않음
    - UDP 소켓에는 송신 버퍼 크기가 있지만 이는 단순히 상한값일 뿐이다.
![그림7](./그림7.png)

#### Standard TCP/IP services
![그림8](./그림8.png)
- 서비스 이름은 /etc/services에 의해 포트 번호에 매핑된다.
----
...
[^MTU]: 패킷 또는 프레임 기반의 네트워크에서 전송될 수 있는 최대 크기의 패킷 또는 프레임을 말한다.
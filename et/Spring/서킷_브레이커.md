...
- `sliding-window-type: COUNT_BASED`: 최근 N개의 호출을 기준으로 실패율을 계산한다. `sliding-window-size: 100`이면 최근 100건의 호출 중 실패 비율을 본다.
- `sliding-window-type: TIME_BASED`: 최근 N초 동안의 호출을 기준으로 실패율을 계산한다. `sliding-window-size: 100`이면 최근 100초간의 호출을 본다.

여기서 `minimum-number-of-calls`도 함께 이해해야 한다. 이 값은 **실패율 계산을 시작하기 위한 최소 호출 수**다. 예를 들어 `minimum-number-of-calls: 50`이면, 호출이 50건 미만일 때는 실패율이 아무리 높아도 서킷을 열지 않는다. 호출이 2건뿐인데 둘 다 실패했다고 실패율 100%로 서킷을 열어버리면 통계적 의미가 없기 때문이다.

## 문제 및 고려 사항

- **폴백(Fallback) 처리**
  - 서킷 브레이커를 통해 작업을 호출하는 애플리케이션은, 작업을 사용할 수 없는 경우를 대비한 폴백 메커니즘을 제공해야 한다. 동일한 작업을 다른 방법으로 수행하거나, 캐시된 데이터를 반환하거나, 사용자에게 예외를 알리고 나중에 다시 시도하도록 유도할 수 있다.
- **예외 유형의 구분**
  - 요청 실패의 원인은 심각도에 따라 다르다. 서킷 브레이커는 예외 유형을 검사하여 전략을 조정할 수 있다. 어떤 예외는 실패로 집계하고(`record-exceptions`), 어떤 예외는 무시할 수 있다(`ignore-exceptions`). 예를 들어 비즈니스 검증 실패(잘못된 입력값)는 대상 서비스의 장애가 아니므로 실패로 집계하지 않는 편이 낫다.
- **모니터링**
  - 서킷 브레이커는 운영팀이 시스템 상태를 평가할 수 있도록 실패한 요청과 성공한 요청 모두에 대한 명확한 관찰 가능성(observability)을 제공해야 한다. 서비스 전반의 엔드 투 엔드 가시성을 위해 분산 추적을 함께 사용한다.

## 서킷 브레이커 말고, 가용성을 위한 다른 선택들

서킷 브레이커는 가용성 패턴 중 하나일 뿐이다. resilience4j는 다음과 같은 보완 패턴들을 함께 제공하며, 실무에서는 보통 조합해서 사용한다.
...
이 패턴들은 서로 배타적이지 않다. 예를 들어 "TimeLimiter로 느린 호출을 끊고 → 서킷 브레이커가 이를 실패로 집계해 차단하고 → 폴백으로 캐시 응답을 내려준다"처럼 계층적으로 결합한다.

## 서킷 브레이커의 적용

### 라이브러리 선택: 왜 resilience4j 인가
...
> 주의: resilience4j 2.x 버전은 Java 17 이상을 요구한다.
...
> `resilience4j-spring-boot3`는 내부적으로 `circuitbreaker`, `annotations` 등 핵심 모듈을 전이 의존성으로 가져오므로, 애노테이션 방식만 쓸 거라면 이 하나로 충분하다.
...
두 방식을 섞으면 **자동 구성 레지스트리와 수동 Bean이 따로 놀게 되어**, 애노테이션이 내가 만든 커스텀 설정을 참조하지 않는 함정에 빠진다. 따라서 한 가지로 통일해야 한다. 이 글에서는 코드가 간결하고 Spring Boot 3의 정석에 해당하는 **애노테이션 방식**으로 진행한다.

### 환경 변수 (application.yml)

애노테이션 방식에서는 `configs`(공통 설정 템플릿)와 `instances`(개별 인스턴스) 구조로 작성한다. 시간 관련 값은 `500ms`, `30s`처럼 단위를 붙여야 자동 구성이 올바르게 바인딩한다.
...
위 YAML에 등장한 옵션들을 한눈에 정리하면 다음과 같다. 크게 **(1) 실패율 집계 기준**, **(2) 서킷을 여는 임계치**, **(3) OPEN/HALF_OPEN 동작**, **(4) 예외 분류** 네 묶음으로 나뉜다.
...
> 기본값은 resilience4j 2.x 기준이며, `configs.default`에 적어두면 모든 인스턴스가 상속한다. 인스턴스별로 다르게 쓰고 싶은 값만 `instances` 아래에서 덮어쓰면 된다.

특히 **실패율(`failure-rate-threshold`)과 느린 호출 비율(`slow-call-rate-threshold`)은 별개의 트리거**라는 점이 중요하다. 호출이 예외 없이 성공하더라도 응답이 `slow-call-duration-threshold`보다 느리면 느린 호출로 집계되고, 그 비율이 임계치를 넘으면 서킷이 열린다. "에러는 안 나는데 다운스트림이 점점 느려지는" 상황을 잡아내는 장치다.

> **핵심 주의 — `record-exceptions` vs `ignore-exceptions`**
> 이 둘은 정반대다. `record-exceptions`는 "이 예외들을 실패로 집계하겠다"는 화이트리스트, `ignore-exceptions`는 "이 예외들은 성공도 실패도 아닌 것으로 무시하겠다"는 블랙리스트다.
> 만약 실패를 일으키는 예외(`RuntimeException`)를 `ignore-exceptions`에 넣어버리면, 그 실패가 통계에서 빠져 실패율이 영원히 0이 되고 **서킷이 절대 열리지 않는다.** 데모에서 흔히 저지르는 실수이니 반드시 구분해서 적자.

### Service

별도의 Config 클래스 없이, YAML에 정의한 인스턴스 이름을 애노테이션에 지정하기만 하면 된다. 인스턴스 이름은 오타를 막기 위해 상수로 관리한다.
...
> 폴백 메서드는 원본 메서드와 동일한 파라미터 목록에 **마지막 인자로 예외 타입을 추가**한 형태여야 한다. 예외 타입을 다르게 하여 오버로딩하면 발생한 예외에 가장 구체적인 시그니처가 선택된다.

### Test

이제 정상 동작을 확인하는 테스트를 작성한다. 단순히 "폴백이 호출되는지"만 보는 것을 넘어서, **서킷 브레이커의 핵심인 상태 전환(CLOSED → OPEN)까지 검증**하는 것이 중요하다. 폴백이 호출되는 것과 서킷이 열리는 것은 별개의 동작이기 때문이다.
...
> 만약 `record-exceptions`를 `ignore-exceptions`로 잘못 설정했다면, 위 세 번째 테스트는 **실패한다.** 실패가 집계되지 않아 서킷이 OPEN으로 가지 않기 때문이다. 즉 이 테스트가 설정 실수를 잡아주는 안전망 역할을 한다.
...
- 서킷 브레이커는 장애 전파를 차단해 **시스템 전체의 가용성을 지키는** 패턴이며, CLOSED / OPEN / HALF_OPEN 세 상태를 오간다.
- 실패율 집계는 Sliding Window(COUNT/TIME)와 `minimum-number-of-calls`로 통제된다.
- OPEN → HALF_OPEN 전환은 기본적으로 **다음 호출 시점**에 일어나며, 자동 전환은 옵션이다.
- 설정 시 `record-exceptions`(실패로 집계)와 `ignore-exceptions`(무시)를 헷갈리지 말 것.
- 애노테이션 방식과 프로그래밍 방식을 **섞지 말고 하나로 통일**할 것.
- 폴백 동작뿐 아니라 **상태 전환까지 테스트**해야 진짜 서킷 브레이커를 검증한 것이다.

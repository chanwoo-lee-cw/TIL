...
> DB 커밋과 Kafka 발행을 atomic하게 묶을 수 없는 문제를, DB 커밋 안에 이벤트 발행 의도를 같이 기록하는 방식으로 해결한다.
...
> 서로 다른 두 저장소(DB, Kafka)에 동시에 쓰는 작업의 원자성을 보장할 수 없는 문제를 **Dual Write Problem**이라고 한다.
...
릴레이 프로세스(폴링 워커 또는 Debezium 같은 CDC)가 추가됩니다. 운영해야 할 컴포넌트가 늘고, 장애 지점도 늘어납니다.

2. 발행 지연(Latency)

폴링 방식이면 폴링 주기만큼, CDC 방식이어도 binlog가 흘러가는 시간만큼 지연이 생깁니다. 실시간성이 매우 중요한 케이스에는 부담입니다.

3. 중복 발행 가능성

릴레이가 발행 후 "발행 완료" 마킹 직전에 죽으면 다음 사이클에서 같은 메시지를 또 보냅니다. 결국 **컨슈머 측 멱등성(idempotency)**이 필수가 됩니다.

4. 순서 보장의 어려움

멀티 인스턴스로 릴레이를 돌리면 발행 순서가 꼬일 수 있고, 같은 aggregate에 대한 이벤트 순서를 보장하려면 파티션 키 설계나 단일 릴레이 같은 추가 제약이 필요합니다.

5. outbox 테이블 관리 비용

계속 쌓이면 테이블이 비대해지고 폴링 쿼리 성능이 떨어집니다. 발행 완료된 row를 주기적으로 아카이빙/삭제하는 배치가 필요해요. 트래픽이 높으면 outbox 테이블 자체가 hotspot이 되기도 합니다.

6. DB 부하 증가

모든 이벤트 발행이 DB write를 동반하니, 메시지 자체를 Kafka에 바로 쏘는 것보다 DB write 부하가 커집니다.
...
DB의 **트랜잭션 로그(MySQL의 binlog, PostgreSQL의 WAL)**를 직접 읽어서 변경 사항을 캡처합니다. 대표적 도구가 Debezium
```
...
Avro + Schema Registry 사용 시 스키마 변경의 어려움:
- 자주 발생하는 케이스들
  - NOT NULL 컬럼을 default 없이 추가 : 구 메시지(discount_amount 필드가 없음)를 읽을 수 없습니다. discount_amount가 NOT NULL인데 구 메시지엔 그 값이 없다.
  - 컬럼 삭제 (FORWARD 호환성 위반) : 구 Consumer가 신 메시지를 읽을 때 legacy_field가 없어서 깨질 수 있음. FORWARD 모드면 거부.
  - 컬럼 타입 변경 : 거의 항상 호환성 위반. 타입이 바뀌면 직렬화 자체가 깨짐.
  - 컬럼명 변경 : Avro 입장에선 "amount 필드 삭제 + total_amount 필드 추가"로 보임. 양쪽 모두 호환성 위반 가능.
## 풀링과 CDC 둘을 어느 상황 때 쓸까?

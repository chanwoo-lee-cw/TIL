...
그럼 보통 밑으로 등분해서 내려갈텐데 보통 a=b 일테니 이런 공식이 필요 없는거 아니냐 싶을 수도 있는데 종종 아닌 경우가 있습니다.

대표적인 공식이  길이가 n인 두 수를 곱하면 시간 복잡도는 $n^2$입니다.

근데, 카라츠바 알고리즘은 곱셉 공식이 학자들이 엄청 연구하는데 답이 n^2으로 내려가지 않아서, 이렇게까지 연구하는데 곱셉의 시간 복잡도는 n^2이 끝인거 아닐까 교수가 강의하고 있을때, 카라츠바라는 학부생이 생각해낸 최초로 n^2 보다 작은 시간 복잡도를 구해낸 걸로 유명한 알고리즘이라고 합니다.
...
이렇게 요약되게 됩니다. 근데 $z_0, z_2$는 중복되는 값이니, $z_0, z_2, z_3$을 각각 한번씩만 구해주면 되는거죠.

즉, 아래로 내려갈 수록 n의 크기는 절반씩 줄어드는데, 부분 문제는 3개로 됩니다.

이런 경우에 마스터 정리를 사용해 구한다면

$T(n) = 3*T(n/2) + O(n) = O(n^{log_23})$

카라츠바 알고리즘의 시간 복잡도가 이렇게 설정됩니다.

즉, 한 단계 아래로 내려갈 때마다 n은 반으로 분할되는데, 구해야 되는 부분 문제는 3개로 나뉘게 됩니다.
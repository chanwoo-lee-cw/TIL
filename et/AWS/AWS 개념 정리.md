...
- Amazon Route 53
  - AWS에서 제공하는 DNS 웹 서비스. DNS란 도메인 네임([www.example.com](http://www.example.com))을 컴퓨터가 소통하는데 사용하는 IP 주소로 변환하여 주는 역할을 말한다.
  - 다음과 같은 주요 기능
...
- web fleet : 들어오는 웹 트래픽을 처리하고 요청을 처리하며 사용자에게 콘텐츠를 전달하기 위해 함께 작동하는 서버 그룹입니다. 웹 플릿은 여러 서버 간에 워크로드를 분산시켜 웹 응용 프로그램 또는 웹 사이트의 성능, 가용성 및 신뢰성을 향상시키는 데 사용됩니다. 웹 플릿은 일반적으로 수신 요청을 플릿의 사용 가능한 서버에 분산하는 로드 밸런싱을 통해 관리됩니다. 로드 밸런싱은 라운드 로빈, 최소 연결 또는 서버 응답 시간과 같은 다양한 알고리즘을 사용하여 수행하여 단일 서버가 트래픽으로 가득 차지 않도록 할 수 있습니다.